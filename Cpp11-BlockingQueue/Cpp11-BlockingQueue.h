...
namespace Async
{
  ...
  public:
    BlockingQueue() {}	//	Void 
    BlockingQueue(BlockingQueue<T>&& bq);	//	Move ctor
    BlockingQueue<T>& operator=(BlockingQueue<T>&& bq);//	Move assignment
    BlockingQueue(const BlockingQueue<T>&) = delete;	//	copy ctor
    BlockingQueue<T>& operator=(const BlockingQueue<T>&) = delete;	// copy assign
    T deQ();	//	deq
    void enQ(const T& t);	//	enq
    T& front();	//	front
    void clear();
    size_t size();
  private:
    std::queue<T> q_;	//	std queue
  ...
  {
	//	locks mutex till the guard object is alive.
