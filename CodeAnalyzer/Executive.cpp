...
#include "../CodePublisher/CodePublisherExecutive.h"
...
void showUsage()
{
	...
	out << "\n	  - z [FILENAME] : write analysis result to file - FILENAME";
	out << "\n	  - y [PATH] : output path for publishing webpages - PATH";
	out << "\n	  - l [WEBPAGE] : webpage to launch automatically - WEBPAGE";
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	try {
		...
		for (int i = 2; i < argc; ++i)
		{
			...
			{
				if (argv[i][1] == 'z') {
					bool valid = setXMLPath(argv[i + 1]); i++;
					if (!valid) {
						showUsage(); return false;
					}
				}
				else if (argv[i][1] == 'y' || argv[i][1] == 'l') {
					storeCommandLineArgs(argv[i][1] == 'y' ? ("outputpath") : ("webpage"), argv[i + 1]); i++;
				}
				else
...
void CodeAnalysisExecutive::setParameters(string path, vector<string> patterns, vector<char> options)
{
	this->path_ = path;
...
bool CodeAnalysisExecutive::storeCommandLineArgs(std::string key, std::string value)
{
	...
	if (key == "outputpath") {
		...
		{
			tuple.second = FileSystem::Path::getFullFileSpec(value);
			cmdLineArgs.insert(tuple);
		}
		else
		{
			tuple.second = getAnalysisPath();
			cmdLineArgs.insert(tuple);
			return false;
		}
	}
...
bool CodeAnalysisExecutive::setXMLPath(char * path) {
	bool validPath = true;
	if (path) {
		int i = 0;
		while (path[i] != '\0') {
...
vector<std::string> CodeAnalysisExecutive::getPatterns() {
	return patterns_;
}

std::string CodeAnalysisExecutive::getCommandLineArgs(std::string key) {
	return cmdLineArgs[key];
}
...
//
//		cout << "------------------------------------------------------------" << endl;
//		CodePublisherRequirementDemo demo;
//		demo.demoAllReqs();
//
//		Config config;
...
//		publisher.publishWebpage(exec.getCommandLineArgs("webpage"));
//		getchar();
//	}
