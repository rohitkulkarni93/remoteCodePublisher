...
*  Maintanence History:
*  --------------------
*  ver 1.5 : 14 March 2017	[ap\rvkulkar]
*  - integrated the dependency analysis engine into the project and triggered it
*	 from this package.
*  - added support to process [FILE] as input in command line required for saving
...
namespace CodeAnalysis
{
	...
	public:
		...
		std::string getCommandLineArgs(std::string key);
		void setParameters(string path, vector<string> patterns, vector<char> options);
	private:
		std::unordered_map<std::string, std::string> cmdLineArgs;
		
		bool storeCommandLineArgs(std::string key, std::string value);
		bool setXMLPath(char * path);
